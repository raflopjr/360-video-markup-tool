<script>
    ...
    // Used for animating camera movement.
    const clock = new THREE.Clock();
    const speed = 2;

    // The rotation the camera should snap to.
    let targetQuaternion = new THREE.Quaternion();
    let targetNote;
    // Boolean when the camera should be locked to a position.
    let shouldBeAnimating = false;
    ...
    function addNote(time, noteContent) {
        ...
        notes.push({
            time,
            noteContent,
            mesh: textContainerMesh,
            duration: DEFAULT_NOTE_DURATION,
            color: DEFAULT_NOTE_COLOR,
            type: "note",
            keyframeColor: "brown",
            position: textContainerMesh.position,
        });
    ...
    function addSnap(time) {
        ...
        direction.multiplyScalar(radius * 0.5);
        let fakeMesh = {
            position: new THREE.Vector3(direction.x, direction.y, direction.z),
        };
        notes.push({
            time,
            noteContent: null,
            ...
            color: DEFAULT_NOTE_COLOR,
            type: "snap",
            keyframeColor: "cadetblue",
            position: fakeMesh.position,
        });
    ...
    function getDefaultKeyframes() {
        DefaultKeyframes.forEach((keyframe, index) => {
            let position = new THREE.Vector3(
                keyframe.position.x,
                keyframe.position.y,
                keyframe.position.z,
            );
            if (keyframe.type === "snap") {
                // recreate the snap.
                addSnap(keyframe.time);

                // Mock structure of mesh for the snapToPosition keyframe.
                notes[index].mesh.position = position;
                notes[index].position = position;
            } else {
                // recreate mesh and save to array.
                addNote(keyframe.time, keyframe.noteContent);

                // Update mesh positon in the scene.
                notes[index].mesh.position.set(
                    position.x,
                    position.y,
                    position.z,
                );
                notes[index].position = position;
                notes[index].mesh.lookAt(0, 0, 0);
            }
        });
    ...
    function snapToNote(note) {
        // The starting rotation before we alter it.
        var startRotation = camera.quaternion.clone();

        // Where we want to be looking at the end.
        camera.lookAt(note.position);
        var endRotation = camera.quaternion.clone();

        // Revert to the starting rotation.
        camera.quaternion.copy(startRotation);

        // Set target and render loop will handle movement.
        targetQuaternion = targetQuaternion.copy(endRotation);
    ...
    function createText(
        ...
        // console.debug(wPxTxt, hPxTxt, wPxAll, hPxAll);
        // console.debug(wWorldTxt, hWorldText, wWorldAll, hWorldContainer);
        return mesh;
    }
    ...
    onMount(() => {
        ...
        function animate(time) {
            ...
            // Time Change since last move.
            const delta = clock.getDelta();

            // Once we reach the position disable the target-locking.
            if (camera.quaternion.equals(targetQuaternion)) {
                shouldBeAnimating = false;
            }
            // While we should be animating move at the speed * time since last change.
            ...
            if(notes.length) {
                // Find notes that are this number of seconds away from the current time.
                const TIME_TO_SNAP_BEFORE = -0.1;
                // TODO: Investigate why this array keeps growing until the video resets.
                let noteToSnapTo = notes.filter(
                    (note) => note.time - video.currentTime < TIME_TO_SNAP_BEFORE
                );
                // If we found a match snap to it if we haven't already.
                if(noteToSnapTo.length && targetNote != noteToSnapTo[noteToSnapTo.length - 1]) {
                    snapToNote(noteToSnapTo[noteToSnapTo.length - 1]);
                    // Keep track we have already snapped to it.
                    targetNote = noteToSnapTo[noteToSnapTo.length - 1]
                }
            }
        ...
        // Load predefined data if in demo mode.
        if (isDemo) {
            loadDemo(true);
        }
    });
